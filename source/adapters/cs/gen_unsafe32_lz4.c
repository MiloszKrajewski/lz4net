...
_next_match:
        ...
        if (0) if ((src_p<src_LASTLITERALS_3) && ((*(uint*)(xxx_ref)) == (*(uint*)(src_p)))) { src_p+=4; xxx_ref+=4; }
    ...
    int dst_len)
{
   ...
   dst_p += STEPSIZE_32-4; xxx_ref -= dec64;
        ...
        {
            if (dst_cpy > dst_end) goto _output_error;
            { do { (*(uint*)(dst_p)) = (*(uint*)(xxx_ref)); dst_p += 4; xxx_ref += 4;; (*(uint*)(dst_p)) = (*(uint*)(xxx_ref)); dst_p += 4; xxx_ref += 4;; } while (dst_p < (oend_COPYLENGTH)); };
