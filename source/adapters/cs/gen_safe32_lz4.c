...
# 55 "..\\..\\..\\original\\lz4.h"
int LZ4_compress(const byte* src, byte* dst, int src_len);
int LZ4_uncompress(const byte* src, byte* dst, int dst_len);
# 83 "..\\..\\..\\original\\lz4.h"
static int LZ4_compressBound(int src_len) { return ((src_len) + ((src_len)/255) + 16); }
# 97 "..\\..\\..\\original\\lz4.h"
int LZ4_compress_limitedOutput(const byte* src, byte* dst, int src_len, int dst_maxlen);
# 111 "..\\..\\..\\original\\lz4.h"
int LZ4_uncompress_unknownOutputSize(const byte* src, byte* dst, int src_len, int dst_maxlen);
# 40 "..\\..\\..\\original\\lz4.c" 2
# 296 "..\\..\\..\\original\\lz4.c"
static int debruijn32[((uint)((uint)((register uint val) & -(register uint val)) * 0x077CB531u)) >> 27]
{
# 319 "..\\..\\..\\original\\lz4.c"
...
_next_match:
        ...
        if (0) if ((src_p<src_LASTLITERALS_3) && (Peek4(_, xxx_ref) == Peek4(_, src_p))) { src_p+=4; xxx_ref+=4; }
        if ((src_p<src_LASTLITERALS_1) && (Peek2(_, xxx_ref) == Peek2(_, src_p))) { src_p+=2; xxx_ref+=2; }
...
_endCount:
  ...
  h = (((Peek4(_, src_p)) * 2654435761u) >> HASH_ADJUST);
        xxx_ref = src_base + hash_table[h];
        hash_table[h] = (int)(src_p - src_base);

        if ((xxx_ref > src_p - (MAX_DISTANCE + 1)) && (Peek4(_, xxx_ref) == Peek4(_, src_p))) { *(xxx_token = dst_p++) = 0; goto _next_match; }
...
_endCount:
  ...
  h = (((Peek4(_, src_p)) * 2654435761u) >> HASH64K_ADJUST);
        xxx_ref = src_base + hash_table[h];
        hash_table[h] = (ushort)(src_p - src_base);
        if (Peek4(_, xxx_ref) == Peek4(_, src_p)) { *(xxx_token = dst_p++) = 0; goto _next_match; }
    ...
    int dst_len)
{
    ...
    while (1)
    {
        ...
        { _i = WildCopy32(_, src_p, _, dst_p, dst_cpy); src_p += _i; dst_p += _i; }; src_p -= dst_p - dst_cpy; dst_p = dst_cpy;
   ...
   dst_p += STEPSIZE_32-4; xxx_ref -= dec64;
        ...
        {
            if (dst_cpy > dst_end) goto _output_error;
            { _i = SecureCopy32(_, xxx_ref, _, dst_p, (oend_COPYLENGTH)); xxx_ref += _i; dst_p += _i; };
            while(dst_p<dst_cpy) *dst_p++=*xxx_ref++;
            dst_p=dst_cpy;
            if (dst_p == dst_end) goto _output_error;
            continue;
        }
        { _i = SecureCopy32(_, xxx_ref, _, dst_p, dst_cpy); xxx_ref += _i; dst_p += _i; };
        dst_p=dst_cpy;
    }
    ...
    int dst_maxlen)
{
    ...
    while (src_p<src_end)
    {
        ...
        xxx_token = *src_p++;
        if ((length=(xxx_token>>ML_BITS)) == RUN_MASK) { int s=255; while ((src_p<src_end) && (s==255)) length += (s=*src_p++); }
        ...
        if ((length=(xxx_token&ML_MASK)) == ML_MASK) { while (src_p<src_end) { int s; length += (s = *src_p++); if (s!=255) break; } }
