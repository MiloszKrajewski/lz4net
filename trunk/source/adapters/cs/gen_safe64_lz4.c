...
# 40 "..\\..\\..\\original\\lz4.c" 2
# 263 "..\\..\\..\\original\\lz4.c"
static int debruijn64[((ulong)((ulong)((register ulong val) & -(register ulong val)) * 0x0218A392CDABBD3FL)) >> 58]
{
# 288 "..\\..\\..\\original\\lz4.c"
...
_next_match:
        ...
        if (1) if ((src_p<src_LASTLITERALS_3) && (Peek4(_, xxx_ref) == Peek4(_, src_p))) { src_p+=4; xxx_ref+=4; }
    ...
    int dst_len)
{
   ...
   dst_p += STEPSIZE_64-4; xxx_ref -= dec64;
        ...
        {
            if (dst_cpy > dst_end) goto _output_error;
            if (dst_p < (oend_COPYLENGTH)) { _i = SecureCopy64(_, xxx_ref, _, dst_p, (oend_COPYLENGTH)); xxx_ref += _i; dst_p += _i; };
            while(dst_p<dst_cpy) *dst_p++=*xxx_ref++;
            dst_p=dst_cpy;
            if (dst_p == dst_end) goto _output_error;
            continue;
        }
        if (dst_p < dst_cpy) { _i = SecureCopy64(_, xxx_ref, _, dst_p, dst_cpy); xxx_ref += _i; dst_p += _i; };
        dst_p=dst_cpy;
    }
