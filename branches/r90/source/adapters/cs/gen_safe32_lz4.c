...
# 473 "..\\..\\..\\original\\lz4.c"
        { _i = dst_p + length; src_anchor += WildCopy32(_, src_anchor, _, dst_p, _i); dst_p = _i; };
    ...
    int dst_len)
{
  ...
  const byte* dst_LASTLITERALS = dst_end - LASTLITERALS;
        ...
        const byte* dst_COPYLENGTH_STEPSIZE_4 = dst_end - COPYLENGTH - (STEPSIZE_32 - 4);
    ...
    while (1)
    {
...

        {
            ...
            if (dst_p < dst_COPYLENGTH) { _i = WildCopy32(_, xxx_ref, _, dst_p, dst_COPYLENGTH); xxx_ref += _i; dst_p += _i; };
        ...
        { _i = WildCopy32(_, xxx_ref, _, dst_p, dst_cpy); xxx_ref += _i; dst_p += _i; };
        dst_p = dst_cpy;
    }
